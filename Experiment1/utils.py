...
class DataFromH5File6(data.Dataset):
    def __init__(self, file_name):
        data = h5py.File(file_name, 'r')
        self.lr = data['low_res']
        self.high_forcing = data['high_forcing']
        self.low_forcing = data['low_forcing']
        
    def __getitem__(self, idx):
        lr = self.lr[idx]
        high_forcing = self.high_forcing[idx]
        low_forcing = self.low_forcing[idx]
        
        return high_forcing, low_forcing, lr
    
    def __len__(self):
        assert self.lr.shape[0] == self.high_forcing.shape[0], "Wrong data length"
        return self.lr.shape[0]